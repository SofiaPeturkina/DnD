...
var TaskShow = /*#__PURE__*/function () {
  function TaskShow(input) {
    _classCallCheck(this, TaskShow);
    this.input = document.querySelector("".concat(input));
    this.addTask();
    this.listTask = _toConsumableArray(document.querySelectorAll('.list-task'));
    this.deleteTask();
    this.showBtnClose();
    this.dragged();
    this.setState();
  ...
  _createClass(TaskShow, [{
    ...
    value: function addTask() {
      ...
      this.input.addEventListener('change', function () {
        ...
        if (!text) return;
        var taskItem = "<li class=\"item-task\">\n                          <span>".concat(text, "</span>\n                          <button class=\"close\">&#9587;</button>\n                          </li>");
        _this.listTask[0].insertAdjacentHTML('beforeend', taskItem);
        _this.input.value = '';
      });
    }
  }, {
    ...
    key: "showBtnClose",
    value: function showBtnClose() {
      document.addEventListener("mouseover", function (e) {
        var close = _toConsumableArray(document.querySelectorAll('.close'));
        var element = e.target.closest('.item-task');
        close.forEach(function (item) {
          item.classList.remove('close_active');
        });
        if (element) {
          var _close = element.querySelector('.close');
          _close.classList.add('close_active');
        }
      });
    }
  }, {
    ...
    value: function dragged() {
      ...
      var onMouseUp = function onMouseUp(e) {
        ...
        var mouseUpItem = e.target.closest('.list-task');
        if (mouseUpItem != null) {
      ...
      document.addEventListener('mousedown', function (e) {
        ...
        if (e.target.closest('.item-task')) {
          ...
          document.querySelectorAll('*').forEach(function (el) {
            return el.style.cursor = 'grabbing';
          });
    ...
    value: function setState() {
      ...
      window.addEventListener('unload', function () {
        var state = {};
        for (var i = 0; i < _this2.listTask.length; i++) {
          var item = _toConsumableArray(_this2.listTask[i].querySelectorAll('span'));
          var content = item.map(function (text) {
    ...
    value: function parseState() {
      ...
      window.addEventListener('load', function () {
        var parse = JSON.parse(localStorage.getItem('item'));
        var _loop = function _loop(key) {
          // this.listTask[key].append(task)
          var arrText = parse[key];
          arrText.forEach(function (item) {
            var task = "<li class=\"item-task\">\n                          <span>".concat(item, "</span>\n                          <button class=\"close\">&#9587;</button>\n                          </li>");
            _this3.listTask[key].innerHTML += task;
            localStorage.clear();
          });
        };
...
var task = new TaskShow('.text-task');
